"""collect_midcap.py — Phase 2 sample expansion: mid/small-cap listed firms
concentrated in sectors with a documented history of PSU-consortium lending
(sugar, textiles, EPC/infra, secondary steel, regional cement, capital goods,
mid chemicals). Reuses the same screener.in scraper as collect_all.py.
Excludes any firm that is itself government/state-owned (confounds firm
ownership with bank ownership) — SAIL, RCF, NFL, GSFC, GNFC, SCI, BHEL, BEML,
MOIL, RVNL, IRCON, MADRASFERT, TNPL are deliberately NOT in this list."""
...
TICKERS = [
 # sugar / agri-processing (classic cooperative -> nationalised-bank legacy)
 "BALRAMCHIN","DHAMPURSUG","TRIVENI","DWARKESH","BAJAJHIND","EIDPARRY","RENUKA",
 # textiles (spinning-heavy, PSU consortium common)
 "RSWM","NAHARSPING","SUTLEJTEX","NITINSPIN","SIYSIL","RUPA","MAFATLAL","GTNTEX",
 "BANSWRAS","SPENTEX",
 # steel/metal secondary & mid (private, not SAIL/RINL)
 "SHYAMMETL","JINDALSAW","RATNAMANI","APLAPOLLO","SARDAEN","KALYANI","MUKANDLTD",
 "USHAMART","GRAVITA","WELCORP","GPIL","FACOR",
 # EPC / infra / construction (heavy PSU consortium lending)
 "NCC","HCC","GRINFRA","PNCINFRA","KNRCON","DBL","JKIL","ASHOKA","IRB","SADBHIN",
 "HGINFRA","PATELENG",
 # cement (regional / mid)
 "ORIENTCEM","SAGCEM","NCLIND","MANGLMCEM","PRISMJOHNS","KESORAMIND",
 # fertiliser / agrochem (private, not RCF/NFL/GNFC/GSFC)
 "ZUARI","SPIC","RALLIS",
 # shipping / logistics (private, not SCI)
 "GESHIP","GATI","SNOWMAN","MAHLOG",
 # paper (small, not TNPL)
 "SESHAPAPER","STARPAPER","ORIENTPPR","ANDHRAPAP",
 # capital goods / power-adjacent private (not BHEL/BEML)
 "TRITURBINE","KIRLOSENG","KIRLPNU","ELECON","TDPOWERSYS","GREAVESCOT",
 # chemicals (mid, beyond the pilot's original 8)
 "GHCL","ALKYLAMINE","VINATIORGA","BALAMINES","NOCIL","GALAXYSURF","DCW",
 "TIRUMALCHM",
 # agri-commodity / edible oil
 "GOKULAGRO","KRBL","LTFOODS","PATANJALI",
 # glass / ceramics / building materials (small)
 "BORORENEW","HINDWAREAP","SOMANYCERA","NITCO",
]
...
def fetch_page(t):
    ...
    if os.path.exists(cache):
        kind = open(meta).read().strip() if os.path.exists(meta) else "consolidated"
...
def grab(table, years, *names):
    ...
    for y in years:
        ...
        for i,h in enumerate(hdr):
            if h == y and vals is not None and i < len(vals):
...
def process(t):
    ...
    if os.path.exists(outp):
        print(t, "SKIP_EXISTS"); return True
    try:
    ...
    if pl is None or bs is None:
        ...
        print(t, "MISSING_SECTIONS_purged"); return False
    ...
    if all(sales[y] == "NA" for y in YEARS):
        for ext in (".html",".meta"):
            fp = os.path.join(HTML, f"{t}{ext}")
            if os.path.exists(fp): os.remove(fp)
        print(t, "EMPTY_TABLES_purged"); return False
...
if __name__ == "__main__":
    ...
    print(f"DONE ok={ok} fail={fail} total={len(TICKERS)}")
