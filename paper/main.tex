...
permutation test; the full-study tables remain pre-specified shells, collected in
Appendix~B pending access to the CMIE Prowess universe and explicitly marked as such.}}
...
bankers}. In the public-data pilot we measure it from annual-report banker lists and,
amount-weighted, from the bank-lender annexures of CRISIL rating rationales; in the
pre-specified full study it comes from the banker field of CMIE Prowess. Either way, since
the 2020 consolidation India has exactly twelve public sector banks, making the
...
our sample is an empirical quantity reported in Table~\ref{tab:p-desc}; the design requires
...
This section and the next pre-specify the \emph{full study}; the public-data pilot that
implements a reduced version of this design is described, with its own data sources, in
Section~\ref{sec:pilot}. Firm-level data for the full study are from the CMIE Prowess
database, the standard source for Indian corporate research. The sample is all NSE/BSE-listed non-financial, non-utility firms with
...
The pre-specified layout of the full study's descriptive-statistics table is
Table~\ref{tab:desc} in Appendix~B; its estimated pilot counterpart is
Table~\ref{tab:p-desc}. Consistent with the selection concerns of
Section~\ref{sec:threats}, we do not expect the clienteles to be balanced in levels ---
PSB-attached firms are anticipated to be older, more capital-intensive, and more levered
--- and the design does not require them to be; it requires stable differences, which the
event studies test.
...
\emph{Targeted fiscal support.} The ATF stabilization fund, export-credit subvention, and guarantee
...
identities transcribed verbatim from company annual reports and CRISIL rating rationales.
Section~\ref{sec:full-cod} then summarizes the \emph{pre-specified full study} on the
complete CMIE Prowess universe; its table shells --- whose structure is frozen before
estimation to discipline specification search, and whose cells are marked ``$\cdot$''
pending data access --- are collected in Appendix~B. The pilot is a proof of concept and a
first, deliberately conservative test of H1--H2; it is not the paper's final evidence, and
its limitations (Section~\ref{sec:pilot-caveats}) are severe enough that no cell below
should be over-read.
...
\label{tab:p-inf}
\small
\input{tables/p_t6_inference.tex}
\begin{tablenotes}\footnotesize
...
\label{tab:p-es}
\small
\input{tables/p_t5_eventstudy.tex}
\begin{tablenotes}\footnotesize
...
\subsection{The pre-specified full study}\label{sec:full-cod}

The full CMIE Prowess study is pre-specified in its entirety, and its frozen table shells
are collected in Appendix~B: descriptive balance across clienteles
(Table~\ref{tab:desc}), the cost-of-debt and capex difference-in-differences of
equation~\eqref{eq:did} with controls and industry$\times$quarter fixed effects added
across columns and a multi-bank subsample (Tables~\ref{tab:cod}--\ref{tab:capex}), the
triple difference of equation~\eqref{eq:ddd} with external-finance-dependence and size
splits (Table~\ref{tab:ddd}), and the H4 allocation tests --- interest-coverage splits,
the composition of post-shock borrowing growth, and investment--cash-flow sensitivity
(Table~\ref{tab:zombie}). Frozen alongside them is the robustness set: (i) continuous PSB
share and main-banker treatments; (ii) entropy-balanced and matched samples on pre-shock
observables; (iii) placebo shocks at FY2024Q3 and FY2025Q3; (iv) exclusion of exporters
(tariff channel) and of any firm receiving scheme-specific support; (v) annual-frequency
replication; (vi) two-way and banker-level clustering; (vii) import-intensity as the
alternative exposure margin. Every cell in Appendix~B is marked ``$\cdot$'' and awaits
estimation; committing the shells before the data disciplines specification search.
...
government-bank relationships is already established \citep{srinivasan2017}.

This paper has done three things. It has shown that corporate--bank relationships in India
can be measured, amount-weighted, from a public source --- the bank-lender annexures of
CRISIL rating rationales --- without proprietary data. It has implemented the crisis test
on that public foundation: across 74 listed firms, the effective cost of debt of
state-bank-attached firms rose by about 0.6--1.1 percentage points less than that of
private-bank firms after the shock, a cushion that appears only post-shock, survives exact
randomization inference for the amount-weighted treatment ($p=0.047$), vanishes at placebo
dates, and --- in a partial allocation test --- accrues to financially stronger rather than
weaker borrowers, the same ``bright side'' pattern the Japanese evidence found. And it has
pre-specified the full CMIE Prowess design --- firm-level exposure, the true ICR~$<1$
zombie margin, thousands of firms including the genuinely constrained ones --- that the
public pilot is powered only to point toward. Whatever the full study finds, the allocation
test carries the interpretation: a cushion for viable constrained firms would extend the
bright side of state banking to a second country and a second century; a cushion for firms
that cannot pay their interest would show that the same aggregate insurance can be written
...
of India press releases for the ATF price-stabilization fund and Export Promotion Mission;
...
\newpage
\section*{Appendix B: Pre-specified table shells for the full study}
\addcontentsline{toc}{section}{Appendix B}
\setcounter{table}{0}
\renewcommand{\thetable}{B\arabic{table}}

\emph{The tables below are the frozen shells of the full CMIE Prowess study summarized in
Section~\ref{sec:full-cod}. Every entry marked ``$\cdot$'' awaits estimation; no figure in
this appendix is a result. The structure is committed before the data to discipline
specification search; any post-hoc addition will be reported as such.}

\begin{table}[h]
\centering
\begin{threeparttable}
\caption{Descriptive statistics by pre-shock banking clientele (full study)}
\label{tab:desc}
\small
...
\centering
\begin{threeparttable}
\caption{State-bank attachment and the cost of debt (full study)}
\label{tab:cod}
\small
\begin{tabular}{lcccc}
\toprule
...
\centering
\begin{threeparttable}
\caption{State-bank attachment and capital expenditure (full study)}
\label{tab:capex}
\small
\begin{tabular}{lcccc}
\toprule
...
\centering
\begin{threeparttable}
\caption{Triple difference: exposure gradients and constraint heterogeneity (full study)}
\label{tab:ddd}
\small
\begin{tabular}{lcccc}
\toprule
...
\centering
\begin{threeparttable}
\caption{Who receives the cushion? Interest-coverage splits and efficiency tests (full study)}
\label{tab:zombie}
\small
\begin{tabular}{lcccc}
\toprule
